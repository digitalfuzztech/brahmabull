...
>
    <button type="button" x-on:click="dropdownOpen = !dropdownOpen; if (dropdownOpen) { $dispatch('brahma-header-dropdown-open', { source: 'team' }) }" x-bind:aria-expanded="dropdownOpen" class="relative rounded-lg p-2 text-white hover:bg-slate-800" aria-label="Open Messenger">
        ...
        <div data-team-dropdown x-cloak x-show="dropdownOpen" x-on:click.outside="dropdownOpen = false" class="fixed left-1/2 top-20 z-[9999] w-[calc(100vw-1.5rem)] max-w-[22rem] -translate-x-1/2 overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl md:absolute md:left-auto md:right-0 md:top-11 md:w-[22rem] md:translate-x-0">
            <div class="flex items-start justify-between gap-3 border-b border-slate-700 px-4 py-3">
                <div><h3 class="font-bold text-white">Messenger</h3><p class="text-xs text-slate-400">Recent authorized conversations</p></div>
                <button type="button" x-on:click="dropdownOpen = false" class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xl leading-none text-slate-400 transition hover:bg-slate-800 hover:text-white" aria-label="Close Messenger">&times;</button>
            </div>
